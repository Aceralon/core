...
func NormalVolumeBindingTestcases(t *testing.T) (testcases []*VolumeBinding) {
	...
	assert.Nil(t, err)
	assert.Equal(t, vb, &VolumeBinding{Source: "/src", Destination: "/dst", Flags: "mrw", SizeInBytes: int64(1000)})
	...
	assert.Nil(t, err)
	assert.Equal(t, vb, &VolumeBinding{Source: "/src", Destination: "/dst", Flags: "mrw", SizeInBytes: int64(0)})
	assert.False(t, vb.RequireSchedule())
	assert.False(t, vb.RequireScheduleMonopoly())
	testcases = append(testcases, vb)
	...
	assert.Nil(t, err)
	assert.Equal(t, vb, &VolumeBinding{Source: "/src", Destination: "/dst", SizeInBytes: int64(0)})
	assert.False(t, vb.RequireSchedule())
	assert.False(t, vb.RequireScheduleMonopoly())
	testcases = append(testcases, vb)
